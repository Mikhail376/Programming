...
/**
 * Модель дракона.
 * Поле owner добавлено для учёта создателя объекта.
 */
public class Dragon implements Comparable<Dragon>, Serializable {
    ...
    public Dragon(long id, String name, Coordinates coordinates, LocalDateTime creationDate,
                  Long age, double wingspan, float weight, DragonCharacter character, DragonHead head, String owner) {
        ...
        if (owner == null || owner.trim().isEmpty()) throw new IllegalArgumentException("Владелец не указан");
    ...
    public String toString() {
        return String.format(
                "Dragon{id=%d, name='%s', %s, age=%s, wingspan=%.2f, weight=%.2f, character=%s, head=%s, owner='%s', created=%s}",
                id, name, coordinates, age, wingspan, weight, character, head, owner, creationDate
        );
    }
}